...
public class Authentication {
     ...
     * @see Authentication#userRegistration(String, Scanner)
     * @see Authentication#userLoggingIn(String, Scanner) */
    public static void userAuthentication() {
        ...
        try {
            if (Objects.requireNonNull(ConnectionToBase.executePreparedStatement("SELECT * FROM USERS WHERE login = ?", login)).next()) {
    ...
    private static void userRegistration(String login, Scanner scanner) {
        ...
        ConnectionToBase.executePreparedStatement("INSERT INTO USERS (login, hash, salt) VALUES (?, ?, ?)", login, Arrays.toString(hash), salt);
    ...
    private static void userLoggingIn(String login, Scanner scanner) {
        ...
        while (userTrying) {
            ...
            try {
                ...
                ResultSet resultSetSalt = ConnectionToBase.executePreparedStatement("SELECT salt FROM USERS WHERE login = ?", login);
                resultSetSalt.next();
                String salt = resultSetSalt.getString(1);
                ResultSet resultSetHash = ConnectionToBase.executePreparedStatement("SELECT hash FROM USERS WHERE login = ?", login);
                resultSetHash.next();
                byte[] hash = md.digest(("*63&^mVLC(#" + scanner.nextLine().trim() + salt).getBytes(StandardCharsets.UTF_8));
